...
function ListPriceBooks() {
  ...
  const handleInputChange = async (id, event) => {
    ...
    if (isNaN(Number_Value)) {
      ...
      try {
        const res = await pricebookService.handleUpdateSale_Price(
          id,
          Number_Value
        );
        let data = res.data;
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            <TableHead>
              <TableRow>
                ...
                <TableCell>
                  {t("pricebook.table.tableHead.sale_price")}
                </TableCell>
              </TableRow>
              <TableRow>
                <TableCell>
                  <TextField
                    size="small"
                    placeholder={t("pricebook.filter.filterCode")}
                  />
                </TableCell>
                <TableCell>
                  <TextField
                    size="small"
                    placeholder={t("pricebook.filter.filterName")}
                  />
                </TableCell>
                <TableCell />
                <TableCell />
